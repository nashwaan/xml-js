...
| `commentKey`        | `"comment"` or `"_comment"` | Name of the property key which will be used for the comment. For example, if `commentKey: '$comment'` then output of `<!--note-->` will be `{"$comment":"note"}` *(in compact form)*  Note: in `json2xml()` function, it is possible to convert multiple comments by adding suffix to the name of the comment property. For example, `{"$comment1": "note1"}` and `{"$comment2": "note2"}` will be converted to `<!--note1-->` and `<!--note2-->`, respectively.  *(in compact form)* |
